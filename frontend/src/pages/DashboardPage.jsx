...
function DashboardPage() {
  ...
  useEffect(() => {
    ...
    fetchUserProgress(user.id, token)
      ...
      .catch(() => {
        if (!active) return;
        setError('Unable to load challenge progress right now.');
      })
      .finally(() => {
        if (active) {
          setLoading(false);
  ...
  const mediumLocked = !progress?.easyCleared;
  const hardLocked = !progress?.mediumCleared;
  ...
  return (
    <>
      ...
      <main className="shell-main shell-scroll">
        ...
        {loading ? (
          <LoadingSpinner label="Loading challenge progress..." />
        ) : (
